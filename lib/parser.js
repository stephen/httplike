...
var parseHeader = function(header) {
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    if (headerline.length >= 2) {
      headers[headerline[0].toLowerCase()] = headerline[1].replace(/^\s+|\s+$/gm, ''); 
    } else {
      output.error = true;
...
var Parser = function(socket, options) {
  ...
  socket.on('data', function(d) {
    ...
    if (collectingContent == -1 && strBuffer.indexOf(HEADER_TERMINATOR) != -1) {
      ...
      if (headerData.error) {
        ...
        if (!headerData.hasContent) {
          ...
          this.emit('message', new MessageData({ headers: headerData.headers, method: headerData.method }), new Response(options));
    ...
    if (collectingContent >= 0 && collectingContent - buffer.length <= 0) { 
      ...
      this.emit('message', new MessageData({ headers: headerData.headers, method: headerData.method, content: content }), new Response(options));
