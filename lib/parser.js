...
var Parser = function(socket) {
  ...
  socket.on('data', function(d) {
    ...
    if (collectingContent == -1 && strBuffer.indexOf(HEADER_TERMINATOR) != -1) {
      ...
      if (headerData.error) {
        ...
        if (!headerData.hasContent) {
          ...
          self.emit('message', new MessageData({ headers: headerData.headers, method: headerData.method }));
    ...
    if (collectingContent >= 0 && collectingContent - buffer.length <= 0) { 
      ...
      self.emit('message', new MessageData({ headers: headerData.headers, method: headerData.method, content: content }));
